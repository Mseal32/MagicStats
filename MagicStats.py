...
dpg.create_context()
dpg.create_viewport(title='Magic Stats')
dpg.setup_dearpygui()
...
def view_stats(sender, value, window):
    ...
    elif options == 'Edit Stats/Delete Decks':
...
def filter_handler(sender, appdata):
    ...
    list_box = dpg.add_listbox(items=['View Player Stats', 'View Decks', 'Edit Stats/Delete Decks'], parent=dpg.last_item(),
                               callback=lambda: view_stats(list_box, player_name, win))
...
def checkbox_vis(sender, item, other_bool):
    if other_bool is True:
...
def create_registry():
    ...
    try:
        with dpg.group(horizontal=True, parent=window):
            box0 = dpg.add_checkbox(label='New Player', callback=lambda: checkbox_vis(box0, 'RegPlayer', dpg.get_value(box1)))
            box1 = dpg.add_checkbox(label='Existing Player', callback=lambda: checkbox_vis(box1, 'Player List', dpg.get_value(box0)))
        ...
        dpg.remove_alias('RegPlayer')
        dpg.remove_alias('Player List')
        dpg.remove_alias('RegDeck')
